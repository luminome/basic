...
class VarsWidget(QTreeWidget):
    ...
    def id_generator(self, root, dict_var, lev=0, ni=0):
        for n, k in enumerate(dict_var):
            ...
            if isinstance(v, dict) or isinstance(v, list):
                ct = 0
                for id_val in self.id_generator(qw, v, lev + 1, n):
                    ct += 1
                    yield lev, id_val
    ...
    def save_data(self, klist, kva=None):
        ...
        def hand(obj, pklist, n=0):
            if isinstance(obj, dict) and n < len(pklist) - 1:
                d_obj = obj[pklist[n]]
                #self.slog(d_obj)
                if isinstance(d_obj, dict):
                    return hand(d_obj, pklist, n + 1)
