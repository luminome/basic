...
class MainPlotWindowHandler:
    ...
    def attach_plot(self, plot):
        ...
        arrow.setPos(0, 0)
        arrow.setStyle(headWidth=(arrow_dims * 0.75), headLen=(arrow_dims * 2), tailLen=arrow_dims,
                       tailWidth=(arrow_dims * 0.5), brush='r')
    ...
    def clickcallback(self, event):
        ...
        b = np.array(delta)
        a = np.array(self.clickpos)
        d = a - b
        #//% measure angle
        ang = np.arctan2(d[1], d[0])
        #print(np.degrees(a))

        self.parent.user_location.setText(f'CLICK({x:.{precision}f},{y:.{precision}f} {np.degrees(ang):.{precision}f}º snap{self.parent.MOUSE_GRID_SNAP})')
    ...
    def show_subplot_bounds(self, subplot_index):
        ...
        caps = pg.GraphItem()
        caps.setData(pos=cap_points, pen=off_pen, brush=off_brush, symbolPen=None, size=[5, 10], symbol=['+', 'x'],
                     pxMode=False)
    ...
    def add_subplot(self):
        ...
        ee = self.t_base.allChildItems()
        self.subplots = [sp for sp in ee if 'GraphItem.GraphItem' in str(sp.__class__)]
...
class GraphUtil(object):
    ...
    def machine_trace(self):
        ...
        target_subplot.setData(pos=pos, pen=None, symbol='o', symbolPen=None, symbolBrush=trace_brush, size=3.0,
                               pxMode=False)
...
class Selecta(QComboBox):
    ...
    def selectionchange(self, i):
        self.index = i
        self.parent.module_change(i, self.currentText())
...
class NewLabel(QLabel):
    ...
    def refresh(self):
        ...
        self.setProperty("mandatoryField", self.is_selected())
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ControlWidget.resizeEvent = self.control_widget_resize_event
    ...
    def control_widget_resize_event(self, event):
        s = self.size()
    ...
    def plotClicked(self, evt, x, y, delta=None):
        ...
        elif self.module_selecta.index == 4:
            #for "texture"
        ...
        if self.module_selecta.index == 5:
            #for "polygon"
    ...
    def module_change(self, index, name):
        #self.module_selecta
    ...
    def mach_make_mark():
        ...
        x, y = MMM.clickpos
        gc += bgc.probe_set_depth(x, y, 1.5)
        point_set = [(np.array(MMM.clickpos) + plush).tolist()]
        gc += bgc.line(point_set, 0)
        MMM.add_gcode_points(gc)
        MACH.load_gcode(gc)
        MACH.delivering = True
        pass
    ...
    @staticmethod
    def mach_registration():
        point_set = [[X_PAGE, 0], [X_PAGE, Y_PAGE], [0, Y_PAGE], [0, 0]]
        crosshair_size_mm = 5.0
        plush = np.array([[-1, 0], [1, 0], [0, 0], [0, -1], [0, 1]]) * crosshair_size_mm
        gc = []

        for i, pcenter in enumerate(point_set):
            x, y = pcenter
            gc += bgc.probe_set_depth(x, y, 1.5)

            point_set = [(np.array(pcenter) + plush).tolist()]
            gc += bgc.line(point_set, 0)
